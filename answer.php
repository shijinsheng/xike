...
<div class="nbrtop clearfix" style="padding-bottom: 11px;">
    ...
    <div class="nbrtopr clearfix" style="width: 140px; position: relative;">
		...
		<div>
		<?php
			$sql_answer1=mysql_query("select * from yourphp_category where id='".$id."'");
			$row_answer1=mysql_fetch_assoc($sql_answer1);
			$sql_answer2=mysql_query("select * from yourphp_article where id='".$pid."'");
			$row_answer2=mysql_fetch_assoc($sql_answer2);
		?>
			<div style="float:right; width:927px;">
            	<div class="clearfix" style="width:100%">
					<div class="nbrbody">
						<div class="lmbk"><?php echo $row_answer1['catname']; ?></div>
						<div class="newsxxtop" style="font-size: 20px; font-family: '微软雅黑';"><?php echo $row_answer2['title']; ?></div>
						<div class="newsxxtime">
							<span>时间:<?php echo date('Y-m-d',$row_answer2['updatetime']); ?></span>
							<span>发布出处:<?php echo $row_answer2['copyfrom']; ?></span>
						</div>
						<p><?php echo $row_answer2['content']; ?></p>
					</div>
				</div>
			</div>
		  </div>
