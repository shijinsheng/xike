    ...
    <script language="javascript" type="text/javascript">
        ...
        $(function () {
            ...
            function InitTable(pageIndex) {                                  
                $.ajax({   
                    type: "POST",  
                    dataType: "text",  
                    url: 'ashx/newstp.ashx',      //提交到一般处理程序请求数据   
                    data: {pageIndex:pageIndex,pageSize:pageSize,kind2:63,kind:63,stat:1},  //提交两个参数：pageIndex(页面索引)，pageSize(显示条数)                   
        ...
        function seach(){
            ...
            $.ajax({   
                ...
                data: {pageIndex:pageIndex,pageSize:pageSize,kind2:63,kind:63,starttime:starttime.value,endtime:endtime.value,key:key.value,count:pcount,stat:1},  //提交两个参数：pageIndex(页面索引)，pageSize(显示条数)                   
                success: function(data) {
                    pcount=data;
                    //分页，PageCount是总条目数，这是必选参数，其它参数都是可选
                    $("#Pagination").pagination(pcount, {
            ...
            function InitTable2(pageIndex) {                                  
                $.ajax({   
                    ...
                    dataType: "text",  
                    url: 'ashx/newstp.ashx',      //提交到一般处理程序请求数据   
                    data: {pageIndex:pageIndex,pageSize:pageSize,kind2:63,kind:63,starttime:starttime.value,endtime:endtime.value,key:key.value,stat:1},  //提交两个参数：pageIndex(页面索引)，pageSize(显示条数)                   
                     success: function(data) {
                         $("#Result").html('');     
                         $("#Result").html(data);  
                     }  
...
<div class="nbrtop clearfix" style="padding-bottom: 11px;">
    ...
    <div class="nbrtopr clearfix" style="width: 140px; position: relative;">
        ...
        <a id="weixinlogo" href="javascript:void(0);">
            ...
            <div class="mtgzban">
              <img   src="images/20130930025940156.JPG"
                    width="939" height="230" />
            </div>
            <form action="javascript:seach();" method="post">
                <div class="mtgzbt2 clearfix" style="border: none; margin-top: 0; height: 30px;">
                    <span>资料搜索：</span>
                    <div style="padding-right: 0;">
                        <input id="starttime" onclick="this.value='';WdatePicker()" type="text" value="开始日期" />
                    </div>
                    <span style="padding-left: 5px; padding-right: 5px;">至</span>
                    <div>
                        <input id="endtime" onclick="this.value = ''; WdatePicker()" type="text" value="结束日期" />
                    </div>
            ...
            <div class="mtgzxw" style="padding-top: 11px; padding-bottom: 36px;">
                <div class="mtgzxw3">
                  <div id="Result">
                    <?php
					$getid = !empty($_GET['typeid'])?$_GET['typeid']:$_GET['id'];
					if($getid){
...
1px solid #E7E7E7;margin-left:0;padding-left:0;padding-bottom:14px;" class="nbrbodynew clearfix">
            <div style="border:none;width:200px;height:142px;" class="nbrbodynewl"><span class="nbrbodynewl" 

style="border:none;width:200px;height:142px;">
<img src="<?php echo $obj["thumb"] ?>" alt="<?php echo $obj["title"] ?>" width="200" height="142" /></span></div>
            <div style="width: 613px;" class="nbrbodynewr">
              <div style="width: 
613px;" class="nbrbodynewr1 clearfix"><a href="newsinfo.php?id=<?php echo $obj["id"] ?>&typeid=<?php echo $getid; ?>"><?php echo $obj["title"] ?></a><span 

style="float: right; padding-left: 0;"><?php echo date("Y-m-d",$obj["createtime"]) ?></span></div>
              <div style="line-height: 21px; padding-top:3px;padding-bottom: 8px;" class="nbrbodynewr2">
	<?php echo $obj["description"] ?>
</div>
              <div class="nbrbodynewr3"><a href="newsinfo.php?id=<?php echo $obj["id"] ?>" style="background:none; color:#c4261d;">查看详细<img src="images/nbrbodynewr3a.png"><img 

src="images/nbrbodynewr3a.png"></a></div>
            </div>
          </div>
          <?php } ?>
          <!--新闻列表结束 -->
                    </div>
                   <!-- <div style="padding-left: 0;" id="Pagination" class="nbrbodyfy clearfix"></div> -->
                </div>
            </div>
        </div>
    </div>
